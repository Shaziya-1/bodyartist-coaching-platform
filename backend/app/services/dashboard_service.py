...
class DashboardService:
    ...
    def get_coach_athlete_detail(self, athlete_id: UUID) -> AthleteDetailResponse:
        ...
        today_meals = self.db.query(MealLog).filter(
            ...
            cast(MealLog.logged_at, Date) == today
        ...
        for m in today_meals:
            ...
            meal_history.append(MealHistoryDetail(
                id=str(m.id),
        ...
        for s in supplement_checklist:
            supplements.append(SupplementDetail(
                name=s.get("name", ""),
        ...
        supp_score = (len(completed_required_supps) / len(required_supps)) * 100.0 if required_supps else 100.0
        # C. Hydration (15%)
        ...
        past_logs = self.db.query(DailyLog).filter(
            DailyLog.athlete_id == athlete_id
        ).order_by(DailyLog.log_date.desc()).limit(15).all()
        ...
        for log in reversed(past_logs_sorted):
            if log.weight is not None and weights_found < 7:
                weight_history.append(ChartPoint(
                    date=log.log_date.strftime("%m-%d"),
        ...
        for log in past_logs_sorted[-7:]:
            water_history.append(ChartPoint(
                date=log.log_date.strftime("%m-%d"),
                value=log.water_logged or 0
            ))
            
        if not water_history:
            water_history = [ChartPoint(date=today.strftime("%m-%d"), value=water_logged)]
